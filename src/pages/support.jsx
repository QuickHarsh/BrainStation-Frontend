...
function Support() {
  ...
  const handleModuleClick = (moduleId) => {
    const selected = parsedUserData.modulePredictions.find(
  ...
  const handleNavigate = async () => {
    if (parsedUserData) {
      const taskData = {
        ...
        strugglingAreas: parsedUserData.lowestTwoChapters.map(
  ...
  return (
    <main className="flex h-screen flex-col items-center justify-between p-10 bg-white">
      <div className="w-full md:w-3/4 bg-white shadow-lg rounded-lg p-6 space-y-6">
        ...
        {/* Selected Module Prediction Section */}
        {selectedModule && (
          <div className="p-4 bg-white shadow-md rounded-lg">
            <p className="text-lg font-bold text-blue-900">
              {selectedModule.predictedExamScore && !isNaN(selectedModule.predictedExamScore) ? (
                ...
                "You are not done any lectures in this module."
              )}
            </p>
          </div>
        )}
            ...
            <div>
              {parsedUserData.lowestTwoChapters.map((chapter, index) => (
                <div key={index} className="mb-6">
                  <p className="text-lg font-bold text-blue-900">
                    You are showing difficulty in <strong className="text-red-700">{chapter.moduleName}</strong> especially in <strong className="text-red-700">{chapter.chapter}</strong> Lecture.
                  </p>
        ...
        {/* Highest and Lowest Scoring Modules Section */}
        <div className="p-4 bg-white shadow-md rounded-lg border-l-4 border-yellow-400">
          ...
          <p className="text-lg font-bold text-blue-900">
            Highest Performance Module 🌟: <strong className="text-blue-700">{parsedUserData.highestScoreModule?.moduleName || "N/A"}</strong>
          </p>
          <p className="text-lg font-bold text-blue-900">
            Lowest Performance Module 💪: <strong className="text-red-700">{parsedUserData.lowestScoreModule?.moduleName || "N/A"}</strong>
          </p>
        </div>
          ...
          {parsedUserData.studyRecommendations && parsedUserData.studyRecommendations.length > 0 ? (
            <ul className="pl-5 list-none">
              ...
              {parsedUserData.studyRecommendations.slice(1).map((rec, index) => (
                <li key={index + 1} className="text-lg font-bold text-blue-900">{rec}</li>
              ))}
            </ul>
          ) : (
