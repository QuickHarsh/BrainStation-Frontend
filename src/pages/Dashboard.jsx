...
import { Chart as ChartJS, ArcElement, CategoryScale, LinearScale, BarElement, PointElement, LineElement } from 'chart.js';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const averageMarks = (studentData.Quiz_IntroductionOS_Marks + studentData['Quiz_Processes&Threads_Marks'] + studentData.Quiz_Deadlocks_Marks + studentData.Quiz_VirtualMemory_Marks) / 4;
        setFocusStudyData([
          { studyTime: 180, focusLevel: studentData.Focus_Level, averageMarks: averageMarks }
        ]);
  ...
  return (
    <div className="dashboard-container" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px', padding: '20px' }}>
      <div className="section" style={{ backgroundColor: 'transparent', color: '#34495e', padding: '20px', borderRadius: '10px' }}>
        <h1 className="text-3xl font-bold">Your Current Progress</h1>
        <p>Completed Tasks: {completedTasksCount}</p>
        <button onClick={handleViewTasks} style={{ backgroundColor: '#f1c40f', color: '#fff', padding: '10px 20px', borderRadius: '5px' }}>
          View Current Task
        </button>
      </div>
